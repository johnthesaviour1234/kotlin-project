...
export default withCors(async function handler(req, res) {
  ...
  try {
    ...
    if (pErr) {
      return res
        .status(500)
        .json(formatErrorResponse('Database error', [{ field: 'database', message: pErr.message }]))
    }
    if (!product) {
    ...
    if (iErr) {
      return res
        .status(500)
        .json(formatErrorResponse('Database error', [{ field: 'database', message: iErr.message }]))
    }

    return res
      .status(200)
      .json(formatSuccessResponse({ ...product, inventory: inv || { stock: 0 } }))
  } catch (e) {
